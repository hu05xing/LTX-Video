...
class CausalVideoAutoencoder(AutoencoderKLWrapper):               # CausalVideo自动编码器程序
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        if (
            ...
            logger.debug(f"✅自动编码器配置文件存在: {config_local_path}")
            logger.debug(f"✅自动编码器配置文件内容: {config}")
            ...
            logger.debug(f"✅自动编码器权重文件路径: {model_local_path}")
        ...
        # 检查vae配置文件并打开配置文件
            ...
            with open(config_path, "r") as f:
                ...
                logger.debug(f"✅vae自动编码器配置文件内容: {config}")
        ...
        # 替换为模型权重安全格式.safetensors加载
            ...
            config = configs["vae"]
            logger.debug(f"✅VAE模型权重safetensors加载成功: {config}")
    ...
    def from_config(config):
        ...
        logger.debug(f"✅VAE模型获取配配置文件关键信息double_z: {double_z}")
    ...
    def __init__(
        ...
        for block_name, block_params in blocks:
            ...
            if block_name == "res_x":
                ...
                logger.debug(f"✅VAW模块unknown块: {block_name}")
        ...
        if latent_log_var == "per_channel":
            ...
            logger.debug(f"✅无效的VAE: {latent_log_var}")
    ...
    def forward(self, sample: torch.FloatTensor) -> torch.FloatTensor:
        ...
        if self.latent_log_var == "uniform":
            ...
            if num_dims == 4:
                ...
                logger.debug(f"❌causal_video编码器模块无效的形状输入: {sample.shape}")
    ...
    def __init__(
        ...
        for block_name, block_params in list(reversed(blocks)):
            ...
            # 创建带注意力机制的残差块
                ...
                logger.debug(f"❌变分编码器未知块类型报错: {block_name}")
